   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      double locked_r = MathMax(0.0, profit_r - InpTrailDistanceR);
      double new_sl = sl;
      if(type == POSITION_TYPE_BUY)
        {
         double candidate = entry + locked_r * risk;
         new_sl = MathMax(sl, MathMax(candidate, entry));
         if(new_sl <= sl)
            continue;
        }
      else
        {
         double candidate = entry - locked_r * risk;
         new_sl = MathMin(sl, MathMin(candidate, entry));
         if(new_sl >= sl)
            continue;
        }
      ...
      if(!g_trade.PositionModify(ticket, new_sl, tp))
         Print(ROBOT_NAME, " trail failed ", g_trade.ResultRetcodeDescription());
      else
         Print(ROBOT_NAME, " trailed SL to ", DoubleToString(new_sl, (int)SymbolInfoInteger(g_symbol, SYMBOL_DIGITS)));
     }
  }
