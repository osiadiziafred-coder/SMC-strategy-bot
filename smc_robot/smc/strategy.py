...
from smc_robot.config import LiquiditySweep, RobotConfig, Signal, Zone
from smc_robot.smc.candles import ensure_ohlc
from smc_robot.smc.fvg import detect_fvg, price_in_zone, unmitigated
from smc_robot.smc.liquidity import detect_liquidity_sweeps, recent_sweeps
...
class SmcStrategy:
    ...
    M5 times the entry after a liquidity sweep and a tap of an unmitigated
    order block or FVG.
    """
    ...
    def evaluate(self, h1, m15, m5) -> Signal | None:
        ...
        sweeps = detect_liquidity_sweeps(m5, cfg.swing_left, cfg.swing_right)
        ...
        if any(e.kind == "BOS" and e.direction == bias for e in h1_events[-3:]):
            ...
            confluence += 1
        if m5_sweeps:
            reasons.append("M5 liquidity sweep")
            confluence += 1
        if zone.kind == "FVG":
