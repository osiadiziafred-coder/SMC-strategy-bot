...
def detect_swings(df: pd.DataFrame, left: int = 2, right: int = 2) -> list[Swing]:
    ...
    A swing high is the first bar of a local peak: strictly higher than `left`
    bars before it, and at least as high as `right` bars after it. Lows are
    the mirror image. Equal wick highs therefore still produce a swing.
    """
    src = ensure_ohlc(df)
