...
def closes_to_candles(closes: list[float], minutes: int, wick: float = 0.25) -> list[Candle]:
...
def structure_from_swings(
    ...
    """Build OHLC so the listed swing points are the unique extrema in their windows."""
...
def bullish_structure_candles(n: int = 96, minutes: int = 60) -> list[Candle]:
    # Confirmed external HH/HL for swing_n_external=5.
...
def m15_buy_setup() -> list[Candle]:
    """Uptrend + confirmed sell-side sweep + displacement (OB/FVG/BOS) + tap."""
    ...
    # FVG at 106: left=ob high 2038.5, right low 2041.2. Last bar taps FVG and stays above OB.
