...
def ranging_chop(start: float = 2000.0, bars: int = 40) -> pd.DataFrame:
    """Sideways series with no displacement, sweep, or fresh zone tap."""
    rows = []
    price = start
    for i in range(bars):
        nxt = price + (0.15 if i % 2 == 0 else -0.15)
        high = max(price, nxt) + 0.08
        low = min(price, nxt) - 0.08
        rows.append((price, high, low, nxt))
        price = nxt
    return ohlc(rows)
